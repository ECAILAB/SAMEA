"""
SAMEA evolutionary kernel for multi-objective EV charging station location
and capacity allocation. The main loop only calls functions.
"""
import os
import sys
import json
...
# =============================================================================
# Non-dominated sort / crowding / selection helpers
# =============================================================================
def non_dominated_sort(F, cv):
    """Fast non-dominated sorting; returns fronts and rank vector."""
    N = F.shape[0]
...
# =============================================================================
def crossover(pop):
    """Uniform column-wise crossover with individual-wise rates."""
...
def mutation(pop):
    """
    Dual-directional mutation: half random, half directed by median F2.
    High F2 biases open/increase; low F2 biases close/decrease.
    """
...
def repair(population):
    """Enforce open-capacity coupling and at least one open station."""
    global Grid_Caps
...
def evaluate_population(population):
    """Evaluate F1 (build cost) and F2 (travel + waiting time)."""
...
def _archive_hv_2d(F):
    """Approximate 2-D HV on normalized archive points (ref = (1.1, 1.1))."""
...
def local_search(attraction_matrix=None, intensity=0.5, gen=0, max_gen=100):
    """
    Multi-objective local search on the archive only.
    Direction follows objective-space position; a trial is accepted only if it
    is not dominated by its parent and does not decrease archive HV.
    """
...
def binary_tournament_indices(n):
    """Return mating indices by binary tournament on rank and crowding."""
...
def _archive_improved(prev_F, curr_F):
    """True if the archive improves in best F1, best F2, or size."""
...
def load_dataset(filename):
    """Load nodes, demands, grid limits, and Manhattan distance matrix."""
...
def build_attraction():
    """Demand-weighted attractiveness matrix A_ij = l_i / (d_ij + 1)."""
...
def environmental_selection(parents_pop, parents_F, parents_CV, offspring, off_F, off_CV):
    """(mu+lambda) environmental selection: merge 2N and keep N."""
    global Pop_Size
...
def inject_elites():
    """
    Replace worst-rank members of the working population by archive elites
    sampled along the front, and reset their crossover/mutation rates.
    """
...
def handle_stagnation():
    """Inject perturbed archive members when stagnation exceeds the threshold."""
...
def binary_tournament_selection():
    """In-place binary tournament selection on the working population."""
...
def update_archive(pop, fit, cv=None):
    """Merge population into the external archive and truncate by crowding."""
...
def initialize_population(attraction_matrix=None):
    """Initialize with light-asset, demand-driven, and high-potential individuals."""
...
def adaptive_control(success_pc=None, success_pm=None):
    """
    Update individual crossover and mutation rates from spatial, temporal,
    and stagnation indicators, smoothed elite baselines, and non-dominated rank.
    """
...
def environmental_selection_1v1(parents_pop, parents_F, parents_CV,
    ...
    """
    One-to-one environmental selection aligned with parent indices.
    Offspring replaces parent if it dominates; under mutual non-dominance,
    the higher crowding distance on the joint set wins (ties broken randomly).
    """
...
def reset_run_state(pop_size):
    """Reset population rates, archive, and stagnation state for a new run."""
...
def default_fit_params():
    """Default parameters for the choice-queuing fitness model."""
    return {
...
def evolve_one_generation(attraction_matrix, gen, max_gen):
    """One generation: mate, crossover, mutation, selection, LS, archive, adapt."""
...
def run(problem_file, pop_size=100, max_gen=300, loop_times=1, verbose=True):
    """Run SAMEA for the given instance over one or more independent trials."""
