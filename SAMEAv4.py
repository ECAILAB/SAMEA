    ...
    for loop in range(loop_times):
        ...
        for gen in range(max_gen):
            ...
            if Stag_Counter >= Stag_Threshold:
                ...
                if n_new > 0:
                    MainPop[-n_new:, 0, :] = np.random.choice([0, 1], size=(n_new, Num_Nodes), p=[0.90, 0.10])
                    for i_new in range(Pop_Size - n_new, Pop_Size):
                        MainPop[i_new, 1, :] = np.where(MainPop[i_new, 0, :] == 1, np.random.randint(1, max(2, Grid_Caps), size=Num_Nodes), 0)
                    MainFit, MainCV = evaluate_population(MainPop)
                Stag_Counter = 0
